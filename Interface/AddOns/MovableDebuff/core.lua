...
local function exec(self, enable)
	...
	if self == TukuiAurasPlayerBuffs then
		...
		if enable then
			...
			if position:match("TOPRIGHT") or position:match("BOTTOMRIGHT") or position:match("BOTTOMLEFT") or position:match("TOPLEFT") then
...
--				debuffs:SetAttribute("wrapYOffset", -67.5)
	...
	if self == TukuiAurasPlayerDebuffs then
		local buffs = TukuiAurasPlayerBuffs
		local debuffs = TukuiAurasPlayerDebuffs
...
debuffs:SetPoint("TOP", TukuiAurasPlayerBuffs, "BOTTOM", 0, -84)
TukuiAurasPlayerDebuffs.backdrop.text:SetText("Move Player Debuffs")
tinsert(T.AllowFrameMoving, TukuiAurasPlayerDebuffs)